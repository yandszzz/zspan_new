...
def analysis_accu(img_base, img_out, ratio, flag_cut_bounds=True, dim_cut=1, choices=4):
    ...
    if choices == 5:
        ...
        CC = C1 / ((C2 * C3) ** 0.5)
        CC = torch.mean(CC)
        return {'SAM': SAM, 'ERGAS': ERGAS, 'PSNR': PSNR, 'SSIM': SSIM, 'CC': CC}  # , q2n_index
...
if __name__ == "__main__":
    ...
    I_fusionnet = sio.loadmat("/home/zds/CODE3070/ZS-Pan_re/result/wv3/19low_1.1.mat")['I_SR']
    I_fusionnet /= 2047.0
    ...
    Qblocks_size = 32;
    dim_cut = 30
    reduce_metrics = analysis_accu(gt[image_id].permute(1, 2, 0), result_our, 4, dim_cut=dim_cut)
    print("---low_evaluate----")
    ...
    I_fusionnet = sio.loadmat("/home/zds/CODE3070/ZS-Pan_test0716/result/wv3/0full.mat")['I_SR']
    ...
    full_metrics = indexes_evaluation_FS(I_F=result_our, I_MS_LR=ms[image_id].transpose(1, 2, 0), I_MS=lms[image_id].transpose(1, 2, 0), I_PAN=pan[image_id].transpose(1, 2, 0),
